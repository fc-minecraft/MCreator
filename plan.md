...
- [x] Кастомный HUD через существующий Overlay элемент MCreator (Реализовано: гибкая привязка оверлея к транспорту, авто-скрытие дефолтного HUD, новые Blockly-блоки получения параметров транспорта: топливо, макс. топливо, тяга, запущен ли двигатель, скорость)
