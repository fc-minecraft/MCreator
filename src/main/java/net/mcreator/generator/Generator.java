...
public class Generator implements IGenerator, Closeable {
	...
	@Nullable
	private ProjectConnection gradleProjectConnection;
	@Nullable
	private GradleFileTracker gradleFileTracker;
	@Nullable
	private GeneratorGradleCache generatorGradleCache;
	...
	@Override
	public @Nonnull Workspace getWorkspace() {
		return workspace;
	}
	...
	/**
	 * Generates the generator mod base files. Formats the imports in the generated
	 * Java code.
	 *
	...
	public boolean generateBase(boolean formatAndOrganiseImports) {
		// Deep Enforcement: Prevent code generation if DRM is missing
		net.mcreator.ui.init.DRMAuthManager.checkEnforcement();

		LOG.info("Generating base files for workspace {}", workspace.toString());
		// Anti-Patching Check: Ensure user is authorized before generating anything
		...
		// (e.g. is a different generator added a file that is no longer present in the
		// generator definition)
		if (workspace.getMetadata("files") instanceof List<?> fileList)
			// filter out files that will be generated by this generator (generatorFiles) as
			// they will be overwritten anyway +
			// at the same time those files may contain user code blocks that will not be
			// preserved if the file is deleted
			// also filter by files in workspace so one can not create .mcreator file that
			// would delete files on computer when opened
		...
		// This handles cases where templates are missing imports
		// (ConcurrentLinkedQueue, Arrays, List, etc.)
		for (GeneratorFile gf : generatorFiles) {
			if (gf.writer() == GeneratorFile.Writer.JAVA) {
				...
				if (f.exists()) {
					...
					if (content.contains("ConcurrentLinkedQueue")
							&& !content.contains("import java.util.concurrent.ConcurrentLinkedQueue;")) {
					...
					if ((content.contains("MethodHandles") || content.contains("MethodType"))
							&& !content.contains("import java.lang.invoke.MethodHandles;")) {
						content = content.replaceAll("(package\\s+[\\w\\.]+;)",
	...
	public List<GeneratorFile> generateElement(GeneratableElement element, boolean formatAndOrganiseImports,
		...
		// Deep Enforcement: Prevent element generation if DRM is missing
		net.mcreator.ui.init.DRMAuthManager.checkEnforcement();

		if (element == null)
			throw new TemplateGeneratorException("Element cannot be null");
		// Anti-Patching Check: Ensure user is authorized before generating element
		net.mcreator.ui.init.DRMAuthManager.validateOrCrash();
		...
		for (GeneratorTemplate generatorTemplate : generatorTemplateList) {
			...
			String code;
			if (generatorTemplate instanceof ListTemplate listTemplate) { // list template - generate it for list data
																			// item pointed at
			...
			// only preserve the last template for given file (only the last template
			// matching given file will be generated)
	...
	public void removeElementFilesAndWorkspaceLinks(GeneratableElement generatableElement) {
		...
		if (map == null) {
			if (generatableElement.getModElement().getType() != ModElementType.UNKNOWN) // silently skip unknown
																						// elements
				LOG.warn("Failed to load element definition for mod element type {}",
		...
		// delete managed tag entries/elements associated with the mod element from the
		// workspace
	...
	@Nonnull
	public List<GeneratorTemplate> getModBaseGeneratorTemplatesList() {
		...
		for (ModElementType<?> type : generatorConfiguration.getGeneratorStats().getSupportedModElementTypes()) {
			...
			} // No need to delete elements here as previous stale files will be removed by
				// used files metadata systems
		}
	...
	private void generateFiles(Collection<GeneratorFile> generatorFiles, boolean formatAndOrganiseImports) {
		...
		for (GeneratorFile generatorFile : generatorFiles) {
			if (generatorFile.writer() == GeneratorFile.Writer.JAVA) {
				// first create Java files if they do not exist already for the import formatter
				// to load them
				...
				if (usercodeComment != null)
					TrackingFileIO.writeFile(workspace,
							UserCodeProcessor.processUserCode(generatorFile.getFile(), generatorFile.contents(),
									usercodeComment),
							generatorFile.getFile());
				else
					TrackingFileIO.writeFile(workspace, generatorFile.contents(), generatorFile.getFile());
			}
		}

		// After we have list of Java files, and they are created, we can format and
		// organise imports in them
	...
	@Nullable
	public ProjectConnection getGradleProjectConnection() {
	...
	@Nullable
	public ProjectJarManager getProjectJarManager() {
