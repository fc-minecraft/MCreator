...
/**
 * Interactive 3D wireframe preview for Entity hitbox alignment.
 * <p>
 * Renders the entity model boxes parsed from a Java or JSON model file,
 * draws a green box representing the hitbox calculated from model width and height.
 * Mouse drag rotates the view, scroll wheel zooms.
 */
public class EntityModelPreviewPanel extends JPanel {
	...
	private float modelWidth = 0.6f;
	private float modelHeight = 1.8f;
	...
	public EntityModelPreviewPanel() {
	...
	public void setHitboxDimensions(double width, double height) {
		modelWidth = (float) width;
		modelHeight = (float) height;
		repaint();
	}
			...
			if (stmt.contains("addBox")) {
				...
				while (bm.find()) {
					try {
						...
						float x = -pF(bm.group(1)) - w, y = pF(bm.group(2)), z = pF(bm.group(3));
						float[][] lc = {
	...
	@Override protected void paintComponent(Graphics g) {
		...
		for (Box3D b : boxes) box3D(g2, b, new Color(150, 150, 150, 15), Theme.current().getForegroundColor(), w, h);

		// Hitbox box (green)
		float hw = modelWidth * 16f;
		float hh = modelHeight * 16f;
		Box3D hitboxBox = new Box3D(-hw / 2f, 24f - hh, -hw / 2f, hw, hh, hw);
		box3D(g2, hitboxBox, new Color(46, 204, 113, 35), new Color(46, 204, 113, 255), w, h);
	}

	private void box3D(Graphics2D g2, Box3D b, Color faceColor, Color edgeColor, int w, int h) {
		Point[] p = new Point[8];
