...
public final class BlocklyJavascriptBridge {
	...
	@SuppressWarnings("unused") public String getMCItemURI(String name) {
		// Deprecated for JCEF, but kept for compatibility
	...
	public void openColorSelector(String color, Consumer<Object> callback) {
		SwingUtilities.invokeLater(() -> {
			...
			String colorString = newColor == null ? null :
					String.format("#%02x%02x%02x", newColor.getRed(), newColor.getGreen(), newColor.getBlue());
			callback.accept(colorString);
		});
	}

	public void openMCItemSelector(String type, Consumer<Object> callback) {
		SwingUtilities.invokeLater(() -> {
			...
			callback.accept(selected != null ? selected.getName() : null);
		});
	}

	public void openAIConditionEditor(String data, Consumer<Object> callback) {
		SwingUtilities.invokeLater(() -> {
	 ...
	 */
	public void openEntrySelector(@Nonnull String type, @Nullable String typeFilter,
			@Nullable String customEntryProviders, Consumer<Object[]> callback) {
